...
bs=25; selected=jobs[batch*bs:(batch+1)*bs]
chunks=Parallel(n_jobs=8,backend='threading',verbose=5)(delayed(one_config)(*j) for j in selected)
rows=[r for ch in chunks for r in ch]
pd.DataFrame(rows).to_csv(f'/mnt/data/ica_batch_{batch:02d}.csv',index=False)
print('saved',batch,len(rows))
raise SystemExit
